...
## 17. Canonical-Parsing Newtype (`MacAddr`)

A step beyond pattern 13: a newtype whose `FromStr` *normalizes* on the way in, so
the same logical value always compares, hashes, and serializes identically no
matter how it was written. `MacAddr([u8; 6])` (`crates/rikitikitavi-models/src/mac.rs`)
parses `aa:bb:..`, `AA-BB-..`, `aabb.ccdd..`, and bare hex to the same six octets.
...
impl FromStr for MacAddr {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        // ...parse exactly 12 hex nibbles into [u8; 6]...
    }
}
```

**Why it matters:** device fingerprints keyed on a *raw string* MAC silently split
one device into two when two scanners format it differently. Canonicalizing in the
type makes that bug unrepresentable. Bit-level accessors (`is_locally_administered`,
`is_multicast`) then live where they belong.

## 18. Embedded, Regenerable Data Tables

For a large static lookup that changes slowly (the 38k-entry IEEE OUI DB, the CISA
KEV catalog), generate a sorted `static &[...]` from an authoritative source with a
script, embed it, and binary-search it — no runtime fetch, works offline.

```rust
// kev_db.rs (generated by scripts/gen_kev_db.py)
#[rustfmt::skip]
static KEV_CVES: &[&str] = &[ "CVE-2002-0367", /* ...1,631 sorted entries... */ ];
pub fn is_kev(cve: &str) -> bool {
    KEV_CVES.binary_search(&cve.trim().to_ascii_uppercase().as_str()).is_ok()
}
```

`#[rustfmt::skip]` keeps the generated table from exploding into thousands of lines
of churn. Volatile data (EPSS scores, ~280k CVEs updated daily) is the opposite
case — fetched best-effort at runtime for just the CVEs a scan found.

## 19. Bounded I/O Primitives Against Hostile Peers

A scanner points at untrusted devices, so every network primitive is bounded by
construction. `http_util::read_body_capped` reads at most `MAX_BODY_BYTES` via
chunked reads (a hostile device can't OOM the scanner with an endless body);
`ports::udp_probe` sends one datagram and waits one timeout, treating no-reply as
